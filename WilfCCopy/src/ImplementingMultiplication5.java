public class ImplementingMultiplication5 {
    ...
    static int[] cutoffs = {3,4,5,6,7,8,10,12,15};
    static int cutoffLength = 9;
...
//I'm assuming you're going to read this one first if you read any at all.
    ...
    static long[] upperRecursion(int maxDistanceLeft, int myMultiplicity, ThreadVariables t) {
        trie myNode = trie.get(t);
        if (myNode != null){
            long[] myCache = myNode.values2[cutoffLength - 1];
            if (myCache != null){
                return myCache;
            } else {
                maxDistanceLeft = myNode.myBaseMaxDistance - (myNode.myBSum) * cutoffs[cutoffLength - 1];
            }
        }
        ...
        if (myNode != null){
            trie.put2(myNode, counts, cutoffLength - 1);
        }
        return counts;
    }
...
//this is the meat of the algorithm, but is virtually the same as upper recursion in explanation
    ...
    static long[] middleRecursion(int maxDistanceLeft, int cutoffNumber, int myMultiplicity, ThreadVariables t) {
        trie myNode = trie.get(t);
        if (myNode != null){
            long[] myCacheValues = myNode.values2[cutoffNumber];
            if (myCacheValues != null) {
                return myCacheValues;
            } else {
                maxDistanceLeft = myNode.myBaseMaxDistance - (myNode.myBSum) * cutoffs[cutoffNumber];
            }
        }
        int lastSeen = 0;
        ...
        if (myNode != null){
            trie.put2(myNode, counts, cutoffNumber);
        }
        return counts;
    }
