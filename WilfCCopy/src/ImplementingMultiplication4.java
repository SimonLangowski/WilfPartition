public class ImplementingMultiplication4 {
...
//I'm assuming you're going to read this one first if you read any at all.
    ...
    static long[] upperRecursion(int maxDistanceLeft, int myMultiplicity, ThreadVariables t) {
        ...
        if (node != null){
            long[] cacheValues = node.values[cutoffLength - 1];
            if (cacheValues != null){
                return cacheValues;
            }
        }
        ...
        for (int b = 1; b <= maxDistanceLeft; b++, baseDistanceLeft -= myMultiplicity) {
            ...
            if (t.increasing[b] == 0) { //this checks if b has not already been used as a base
                ...
                if (myMultiplicity + 1 <= maxDistanceLeft) { //check if m + 1 is within my range
                    //recurse for m + 1
                    moreValues = upperRecursion(maxDistanceLeft,myMultiplicity + 1, t);
                    for (int k = myMultiplicity + 2; k <= maxDistanceLeft; k++) {
                        long[] moreMoreValues = upperRecursion(maxDistanceLeft,k, t);
                        for (int l = 0; l <= baseDistanceLeft; l++) {
                            moreValues[l] = moreValues[l] + moreMoreValues[l];
                        }
                    }
                }
                long[] values = middleRecursionWrapper(maxDistanceLeft,cutoffLength - 2, t); //only ask for what we need
                {
        ...
        if (node != null){
            trie.put(node, counts, cutoffLength - 1);
        }
        return counts;
    }
...
//by adding the wrapper of the cutoff value one less
    static long[] middleRecursionWrapper(int maxDistanceLeft, int cutoff, ThreadVariables t) {
        if (cutoff >= 0) {
            long[] a = middleRecursionWrapper(maxDistanceLeft,cutoff - 1, t);
...
//this is the meat of the algorithm, but is virtually the same as upper recursion in explanation
    ...
    static long[] middleRecursion(int maxDistanceLeft, int cutoffNumber, int myMultiplicity, ThreadVariables t) {
        trie node = trie.get(t);
        if (node != null) {
            long[] cacheValues = node.values[cutoffNumber]; //values from wrapper (Sum of all multiplicities less than)
            if (cacheValues != null){
                return cacheValues;
            }
        }
        ...
        for (int b = 1; b <= maxDistanceLeft; b++, baseDistanceLeft -= myMultiplicity) {
            ...
            if (t.increasing[b] == 0) { //maybe we can replace cacheValues when currentDistanceLeft drops below the highest current base -> and then modify the lookup function to accept a start value (or index)
                ...
                if (myMultiplicity + 1 < cutoffs[cutoffNumber + 1]) { //check if m + 1 is within my range
                    //recurse for m + 1
                    moreValues = middleRecursion(maxDistanceLeft,cutoffNumber, myMultiplicity + 1, t);
                    for (int k = myMultiplicity + 2; k < cutoffs[cutoffNumber + 1]; k++) {
                        long[] moreMoreValues = middleRecursion(maxDistanceLeft,cutoffNumber, k, t);
                        for (int l = 0; l <= baseDistanceLeft; l++) {
                            moreValues[l] =  moreValues[l] + moreMoreValues[l];
                        }
                    }
                ...
                long[] values = middleRecursionWrapper(maxDistanceLeft,cutoffNumber - 1, t); //only ask for what we need
        ...
        if (node != null){
            trie.put(node, counts, cutoffNumber);
        }
        return counts;
    }
    ...
    static long[] lowerRecursion(int maxDistanceLeft, int myMultiplicity, ThreadVariables t) {
        ...
        for (int b = 1; b <= maxDistanceLeft; b++, baseDistanceLeft -= myMultiplicity) {
            ...
            if (t.increasing[b] == 0) {
                addSorted(t, b, lastSeen);
                long[] values = q2All(maxDistanceLeft,t);
                long[] moreValues = null;
                //increment m
                //need to run on all values of m up to cutoff
                if (myMultiplicity + 1 < cutoffs[0]) { //check if m + 1 is within my range
                    //recurse for m + 1
                    moreValues = lowerRecursion(maxDistanceLeft,myMultiplicity + 1, t);
                    for (int k = myMultiplicity + 2; k < cutoffs[0]; k++) {
                        long[] moreMoreValues = lowerRecursion(maxDistanceLeft, k, t);
                        for (int l = 0; l <= baseDistanceLeft; l++) {
                            moreValues[l] = moreValues[l] + moreMoreValues[l];
                        }
                    }
...
/*
    static long[] distanceFrom(int maxDistanceLeft, int cutoff, ThreadVariables t){



    }

    static long[] countEvenWithout(int maxDistanceLeft, int cutoff, ThreadVariables t){

    }

    static long[][] countAll(int maxDistanceLeft, int minMult, int maxMult){
        if (maxDistanceLeft == 0){
            long[][] vals = new long[1][1];
            vals[0][0] = 1;
            return vals;
        }
        long[][] total = new long[maxDistanceLeft][MAX];
        //can call countEven and countOdd again?

        //these should be given mutually exclusive multiplicity ranges
        long[][] evenSmall = countEven(maxDistanceLeft, cutoff - 1);
        long[][] oddSmall = countOdd(maxDistanceLeft, cutoff - 1); //since this is just going to call evenSmall with the same parameters just called, something should be cached
        //possibilities
        //no number corresponds to multiplicity cutoff

        //an even number corresponds to multiplicity cutoff


        //an odd number corresponds to multiplicity cutoff

        //I'm confused how this accounts for smaller parts that are a mix of even and odd numbers
    }

    static void addToArray(long[][] a, long[][] b){
    ...
    static long[][] countEven(int maxDistanceLeft, int minMult, int maxMult){ //- simply all the ways to make up through distLeft/2 -> and then doubled?
        long[][] all = countAll(maxDistanceLeft / 2, minMult, maxMult);
        long[][] even = new long[maxDistanceLeft][MAX]; //the maximum m sum is max because each is paired with at least one
        //need to double values and base sums, mult sums stay the same

        return even;
    }

    static long[][] countOdd(int maxDistanceLeft, int minMult, int maxMult){ //- all the even ways, and then shifted one
        long[][] even = countEven(maxDistanceLeft, minMult, maxMult);
        long[][] odd = new long[maxDistanceLeft][MAX];

        //need to increase all bases by 1, increasing the values by the mult sums (the mult sums stay the same, and the bsum increases by numTerms)


        for (int i = 0; i < )
    }

    //if we had base sums, then the mult ranges could be calculated by shifting as well
    */
}
